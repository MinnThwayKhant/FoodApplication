    ...
    const {idMeal, strMeal: title, strMealThumb:image} = meal
        return(
            <div key={meal.idMeal} className='shadow-2xl rounded-lg overflow-hidden w-110'>
                <div>
                    <img src={image} alt="" className='object-cover w-full h-60'/>
                </div>
                <div className='m-4 flex justify-between'>
                    <h1 className='text-lg'>{title}</h1>
                    <button className='text-amber-900 cursor-pointer text-lg hover:opacity-80 transition-transform duration-300 hover:-translate-y-0.5'><FaRegThumbsUp className='text-xl'/></button>
                </div>
            </div>
        )
